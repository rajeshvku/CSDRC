...
{
    private static void Mainh(string[] args)
    {
        ...
        //using StreamWriter sw = new StreamWriter("E:\\Personal\\CSDRC\\CSDRC.TestWix\\NewWixFile.wix");
        ...
        foreach (var table in tabless)
        {
            ...
            //}
            if (tableName == "ActionText")
            {
                using StreamWriter sw = new StreamWriter($"E:\\Personal\\CSDRC\\CSDRC\\sample\\{tableName}.wix");
                sw.WriteLine($"<{tableName}>");
                var reows = table.Elements("row");
                List<ActionText> isPathVariableList = table.Descendants("row").Select(row =>
                new ActionText
                {
                    Action = row.Element("td")?.Value,
                    Description = row.Elements("td").Skip(1).FirstOrDefault()?.Value,
                    Template = row.Elements("td").Skip(2).FirstOrDefault()?.Value
                }).ToList();
                foreach (var pathVariable in isPathVariableList)
                {
                    var stringVariable = $"<ProgressText Action=\"{pathVariable.Action}\" Message =\"!(loc.{pathVariable.Description})\" Template=\"!(loc.{pathVariable.Template})\" />";
                    sw.WriteLine(stringVariable);
                }
                sw.WriteLine($"</{tableName}>");
                sw.Close();
            ...
            if (tableName == "AdminExecuteSequence" ||tableName == "AdminUISequence" ||tableName == "InstallExecuteSequence"||tableName == "AdvtExecuteSequence")
            {
                using StreamWriter sw = new StreamWriter($"E:\\Personal\\CSDRC\\CSDRC\\sample\\{tableName}.wix");
                sw.WriteLine($"<{tableName}>");
                var reows = table.Elements("row");
                List<AdminExecuteSequence> isPathVariableList = table.Descendants("row").Select(row =>
                new AdminExecuteSequence
                {
                    Action = row.Element("td")?.Value,
                    Condition = row.Elements("td").Skip(1).FirstOrDefault()?.Value,
                    Sequence = row.Elements("td").Skip(2).FirstOrDefault()?.Value,
                    ISComments = row.Elements("td").Skip(3).FirstOrDefault()?.Value,
                    ISAttributes = row.Elements("td").Skip(3).FirstOrDefault()?.Value
                }).ToList();
                foreach (var pathVariable in isPathVariableList)
                {
                    var stringVariable = $"<{pathVariable.Action} Sequence=\"{pathVariable.Sequence}\"/>";
                    sw.WriteLine(stringVariable);
                }
                sw.WriteLine($"</{tableName}>");
                sw.Close();
            ...
            //if (tableName == "AdminUISequence")
            //{
            //    using StreamWriter sw = new StreamWriter($"E:\\Personal\\CSDRC\\CSDRC\\sample\\AdminExecuteSequence.wix");
            //    sw.WriteLine($"<AdminUISequence>");
            //    var reows = table.Elements("row");
            //    List<AdminUISequence> isPathVariableList = table.Descendants("row").Select(row =>
            //    new AdminUISequence
            //    {
            //        Action = row.Element("td")?.Value,
            //        Condition = row.Elements("td").Skip(1).FirstOrDefault()?.Value,
            ...
            //    StreamWriter sw = new StreamWriter("C:\\Users\\Rajesh\\Documents\\wixMessagea.txt");
            ...
            //        var newTag = $"<String Id=\"{splittedLi[0]}\">{valuestr}</String>";
